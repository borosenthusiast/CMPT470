...
Profile.prioritizeChosenId = async(ids) => {
	var userid = ids.userId;
	var chosenid = parseInt(ids.chosenId, 10);

	try{
		let profile_collection = await mongo_db.mongo_collection('ProfilePage');

		let query = {userId: userid};
		let pull = { $pull: {addedlist: chosenid} }; //remove chosenid from addedlist
		let push = { $push: {addedlist: chosenid} }; //remove chosenid from addedlist
		let projection = {userId: 1, addedlist:1 };
		
		let pullresult = await profile_collection.updateOne(query, pull, projection);
		//console.log(pullresult.addedlist);

		let pushresult = await profile_collection.updateOne(query, push, projection);
